...
transformData <-
  function(data,
    ...
    if (showOccurrance)
    {
      returnData <- dplyr::arrange(dplyr::ungroup(dplyr::mutate(dplyr::group_by(dplyr::arrange(returnData, SUBJECT_ID, STATE_START_DATE), SUBJECT_ID, STATE_LABEL), OCCURRENCE = dplyr::row_number())), SUBJECT_ID, STATE_START_DATE)
